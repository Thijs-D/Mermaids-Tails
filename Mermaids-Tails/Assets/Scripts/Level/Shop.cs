...
public class Shop : MonoBehaviour
{
    ...
    {
        GameStats.gameStatsRef.getPlayer().GetComponent<Player>().setWeapon(meleeDmg ,minDmg, maxDmg, projectileSpeed);
    }
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.GetComponent<Player>() && GameStats.gameStatsRef.getCoins() >= cost)
        {
            ...
            meleeDmg = GameStats.gameStatsRef.getPlayer().GetComponent<Player>().getMeleeDmg() * 2;
